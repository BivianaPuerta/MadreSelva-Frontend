...
const ActualizarProducto = () => {
    ...
    fetch(`http://localhost:8080/api/cosmetics/${reference}`)
    .then((res) => res.json())
    .then((data) => {
    ...
    const { brand, price, category, quantity, description, name, photography } = productosActualizar;
    ...
    const formik = useFormik({
        ...
        onSubmit: datos => {
            Swal.fire({
                ...
                confirmButtonText: 'Desea Actualizar!'
              }).then((result) => {
                if (result.isConfirmed) {
                    try{ console.log(datos);
                        fetch(`http://localhost:8080/api/cosmetics/update`, {
                            method: "PUT",
